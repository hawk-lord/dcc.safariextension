...
if (!this.DirectCurrencyContent) {
    const DirectCurrencyContent = (function(aDccFunctions) {
        ...
        const makePriceRegexes = function() {
            new Promise(
                function(resolve, reject) { PriceRegexes? resolve(PriceRegexes): reject(Error("makePriceRegexes rejected")) }
            ).then(
                function(aPriceRegexes) { aPriceRegexes.makePriceRegexes(regex1, regex2)},
                function(err) { console.error("makePriceRegexes then error " + err) }
            ).catch(
                function(err) { console.error("makePriceRegexes catch error " + err) }
            );
        };
        ...
        const replaceCurrency = function(aNode) {
            ...
            var convertedContent = aNode.nodeValue;
        ...
        const substituteOne = function(aNode, isShowOriginal, aDccTitle) {
            if (!aNode) {
                return;
            }
            if (!aNode.parentNode) {
        ...
        const substituteAll = function(aNode, isShowOriginal) {
            if (!aNode) {
                return;
            }
            const nodeList = aNode.querySelectorAll(".dccConverted");
         ...
         */
        const readParameters = function(contentScriptParams) {
        ...
        const readEnabledCurrencies = function(contentScriptParams) {
        ...
        const onUpdateSettings = function(contentScriptParams) {
            ...
            const startConversionWhenReady = function() {
                new Promise(
                    function(resolve, reject) { PriceRegexes ? resolve(): reject(Error("startConversionWhenReady Not OK"))}
                ).then(
                    startConversion(),
                    function(err) { console.error("startConversionWhenReady then error " + err)}
                ).catch(
                    function(err) { console.error("startConversionWhenReady catch error " + err)}
                );
            };
